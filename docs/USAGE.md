...
| **中央** | ツールバー（ツール切替・マグネット・線幅・明るさ・コントラスト・フィット・元に戻す・塗り表示・保存。アノテーションを選択中は「削除」も出ます）とキャンバス。最下部にその場面で使えるキーのヒントバー |
...
ツールバーの **「外接枠」** ボタンを押すと、確定したライン・多角形を囲む**破線の外接ボックス**の表示を切り替えられます（既定は ON）。
これは検出用エクスポート（YOLO 検出）でライン・多角形に自動で付く枠と**同じもののプレビュー**で、表示するだけです。アノテーションのデータは増えも減りもしません。

マグネット・反転・外接枠の設定は `_anno/project.json` に保存され、次にそのフォルダを開いたときの初期値になります。
...
| **まるごと削除** | `Delete`、または `Ctrl` / `Cmd` + `Backspace`、ツールバーの **「削除」** ボタン（`Ctrl` / `Cmd` + `Z` で元に戻せます） |
...
| `Delete`、または `Ctrl` / `Cmd` + `Backspace` | 選択中のアノテーションを丸ごと削除 |
| ツールバーの **「削除」** ボタン | 同上（選択しているときだけ表示されます） |
| `1`〜`9` | 選択中のアノテーションのクラスを変更 |

削除しても `Ctrl` / `Cmd` + `Z` で元に戻せます（100 手前まで）。

> **MacBook をお使いの方へ**: MacBook のキーボードには Forward Delete キーがありません（`delete` と書かれたキーの実体は `Backspace` です）。
> 丸ごと削除は **`⌘` + `delete`**（= `Cmd` + `Backspace`）か、ツールバーの「削除」ボタンを使ってください。
> 修飾キーなしの `delete` はこれまでどおり「1 つ戻す」のままです。
> なお描画の途中は `⌘` + `delete` を押しても何も起きません（描いている途中の図形を捨てるのは `Esc` です）。
...
| `F` | 画面にフィット |
| `T` | 塗りつぶし表示の ON / OFF |
| ツールバーの「外接枠」 | ライン・多角形の**外接ボックス**（破線）の表示 ON / OFF。既定 ON。検出用エクスポートで自動付与される枠と同じもので、**表示のみ**です |
...
| `Delete`、または `Ctrl` / `Cmd` + `Backspace` | 選択中のアノテーションを全体削除（`Ctrl` / `Cmd` + `Z` で戻せます）。MacBook は `⌘` + `delete` |
...
### アノテーションを削除できない（MacBook で `Delete` キーが見つからない）

MacBook のキーボードには **Forward Delete キーがありません**。`delete` と書かれているキーの実体は `Backspace` で、GenbaAnno ではこれが「1 つ戻す」に割り当たっています。丸ごと削除したいときは次のどれかを使ってください。

- **`⌘` + `delete`**（= `Cmd` + `Backspace`）… `Delete` と同じ「選択中のアノテーションを丸ごと削除」です。
- **ツールバーの「削除」ボタン** … 編集モード（`V`）でアノテーションを選択すると出てきます。
- **右パネルのアノテーション一覧のゴミ箱ボタン**。
- `fn` + `delete`（= Forward Delete）でも従来どおり削除できます。

いずれの場合も、まず編集モード（`V`）で対象をクリックして**選択**してから操作してください。選択していないと何も起きません。
消してしまっても `Ctrl` / `Cmd` + `Z` で元に戻せます。

なお、**描いている途中**（まだ確定していない図形がある状態）では `⌘` + `delete` は反応しません。誤って作りかけの図形ごと消してしまわないための仕様です。描画中のものを捨てたいときは `Esc`、1 点だけ戻したいときは `delete`（修飾キーなし）を使ってください。
